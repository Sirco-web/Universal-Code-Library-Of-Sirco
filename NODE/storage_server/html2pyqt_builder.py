...
class FTPWebClient(QMainWindow):
    def __init__(self, html_path):
        super().__init__()
        self.setWindowTitle("FTP Web Client (Python Builder)")
        self.resize(900, 600)
...
# TROUBLESHOOTING QT "xcb" ERROR:
# -------------------------------------------------
# The error:
#   qt.qpa.plugin: Could not load the Qt platform plugin "xcb" in "" even though it was found.
# means your system is missing required libraries for Qt GUI apps.

# To install all required Qt/GUI dependencies for PyQt5 on Ubuntu/Debian, run:
# sudo apt-get update && sudo apt-get install -y libxcb-xinerama0 libxcb-xinerama0-dev libxkbcommon-x11-0 libxcb1 libx11-xcb1 libxcb-render0 libxcb-shape0 libxcb-shm0 libxrender1 libxi6 libxtst6 libxcomposite1 libxcursor1 libxdamage1 libxfixes3 libxrandr2 libxss1 libgl1-mesa-glx libgl1-mesa-dri libgl1

# And install Python dependencies:
# pip install PyQt5 requests

# After installing, restart your terminal/session and re-run:
# python html2pyqt_builder.py

# If you are running in a headless environment (no display), you cannot use PyQt5 GUI apps.
# You must run this on a machine with a graphical environment (or use X11 forwarding/VNC).

# This is not a Python code bug, but a missing system dependency for Qt.

def main():
    from PyQt5.QtWidgets import QFileDialog
    app = QApplication(sys.argv)
    # Ask user for HTML file path if not provided as argument
    if len(sys.argv) < 2:
        file_dialog = QFileDialog()
        file_dialog.setFileMode(QFileDialog.ExistingFile)
        file_dialog.setNameFilter("HTML Files (*.html *.htm)")
        file_dialog.setWindowTitle("Select index.html")
        if file_dialog.exec_():
            html_files = file_dialog.selectedFiles()
            if not html_files:
